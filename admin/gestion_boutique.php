...
if(isset($_GET['action']) && $_GET['action'] == "suppression")
{
    ...
    $contenu .= '<div class="alert alert-success text-center" role="alert">✅ Suppression du produit n° : ' . $_GET['id_produit'] . ' réalisé avec succès ! </div>';
...
$contenu .= '<div class="col-md-6"><button type="button" class="btn btn-primary"><a href="?action=ajout">Ajouter un produit</a></button></div></div></div><br><br><hr>';
//------------------------------------- AFFICHAGE TABLEAU PRODUIT -------------------------//
if(isset($_GET['action']) && $_GET['action'] == "affichage")
{
    ...
    while($ligne = $resultat->fetch_assoc())
    {
        ...
        $contenu .= '<td class="text-center"><button class="btn btn-info"><a href="?action=modification&id_produit=' . $ligne['id_produit'] . '"><i class="far fa-edit"></i></a></button></td>';
        $contenu .= '<td class="text-center"><button class="btn btn-dark"><a href="?action=suppression&id_produit=' . $ligne['id_produit'] . '"Onclick="return(confirm(\'Vous êtes sur le point de supprimer ce produit. En êtes vous certain ?\'));"><i class="far fa-trash-alt"></i></a></button></td>';
        $contenu .= '</tr>';
    }
...
if(isset($_GET['action']) && ($_GET['action'] == 'ajout' || $_GET['action'] == 'modification'))
{
    ...
    <input type="text" class="form-control" name="reference" id="reference" placeholder="Lé référence du produit" value="'; if(isset($produit_actuel['reference'])) echo $produit_actuel['reference']; echo'">
    </div>
    <div class="mb-3">
    ...
    <textarea class="form-control" name="description" id="description" cols="30" rows="10" placeholder="La description du produit">';if(isset($produit_actuel['description'])) echo $produit_actuel['description']; echo'</textarea>
    </div>
    <div class="mb-3">
    ...
    <input type="radio" name="public" value="mixte"'; if(isset($produit_actuel) && $produit_actuel['public'] == 'mixte') echo ' checked '; echo '> Mixte  
    </div>
    <div class="mb-3">
    ...
    <input type="text" class="form-control" name="stock" id="stock" placeholder="Le stock disponible pour ce produit" value="';if(isset($produit_actuel['stock'])) echo $produit_actuel['stock']; echo '">
    </div>
